...
f(X) = 3*X[1]^2+12/X[1]^3-5 #0.5-2.5
fg(x) = 3*x^2+12/x^3-5 #0.5-2.5
...
function graph(a)
    anim = @animate for i in eachindex(a)
        plot(fg,0.5,2.5,label="f(x)")
        scatter!([a[i]],[f(a[i])],color="red",label="x")
    end

    gif(anim, "/Users/lloydna/Desktop/UP/5° Semestre/Optimizacion/Optimizacion/Tareas/Tarea4/Parte1/Ejercicio1/GIFS/Newton.gif", fps = 15)
end
