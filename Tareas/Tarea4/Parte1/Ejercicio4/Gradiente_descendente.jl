...
f(x) = 3*x^2+12/x^3-5 #0.5-2.5

function graph(a)
    anim = @animate for i in eachindex(a)
        plot(f,0.5,2.5,label="f(x)")
        scatter!([a[i]],[f(a[i])],color="red",label="x")
    end

    gif(anim, "/Users/lloydna/Desktop/UP/5° Semestre/Optimizacion/Optimizacion/Tareas/Tarea4/Parte1/Ejercicio4/GIFS/Gradiente.gif", fps = 15)
end
...
function Gradiente() #funciona para cualquier dimension modificando la cantidad de variables en la funcion
    X::Float64 = 1.5 #vector de valores iniciales (aleatorio)
    alpha = 0.05 #tasa de aprendizaje (de crecimiento)
    iteraciones = 100
    ThisX = [copy(X)]
